 ...
 * @param params.username - 비밀번호를 재설정할 사용자의 사용자 이름.
 * @param params.code - 비밀번호 재설정을 위해 사용자에게 전송된 확인 코드.
 * @param params.password - 사용자에게 설정할 새 비밀번호.
 * @returns {Promise<ResetPasswordResponse>} 성공 메시지로 resolve되거나
 * 오류 메시지로 reject되는 Promise.
 *
 * @throws 비밀번호 재설정에 실패할 경우 오류 메시지를 throw합니다.
 */
